 ...
 */
package com.young.tools.miner.cluster.mahout.kmeans;
...
import com.young.tools.miner.cluster.mahout.WeightedPropertyVectorWritable;
...
public class KMeansClusterer {
	...
	private static final Logger log = LoggerFactory
			.getLogger(KMeansClusterer.class);

	/** Distance to use for point to cluster comparison. */
	private final DistanceMeasure measure;

	/**
	 * Init the k-means clusterer with the distance measure to use for
	 * comparison.
	 * 
	 * @param measure
	 *            The distance measure to use for comparing clusters against
	 *            points.
	 * 
	 */
	 ...
	 * point. Distance measure used is configured at creation time.
	 * 
	 * @param point
	...
	public void emitPointToNearestCluster(Vector point,
		...
		for (Cluster cluster : clusters) {
			...
			double distance = this.measure.distance(
					clusterCenter.getLengthSquared(), clusterCenter, point);
	...
	public void outputPointWithClusterInfo(
		...
		for (Cluster cluster : clusters) {
			...
			double distance = measure.distance(
					clusterCenter.getLengthSquared(), clusterCenter, vector);
			if (distance < nearestDistance || nearestCluster == null) {
				nearestCluster = cluster;
				nearestDistance = distance;
			}
		...
		props.put(new Text("distance"),
				new Text(String.valueOf(nearestDistance)));
	...
	protected void emitPointToNearestCluster(Vector point,
			Iterable<Cluster> clusters, Writer writer) throws IOException {
		Cluster nearestCluster = null;
		double nearestDistance = Double.MAX_VALUE;
		for (Cluster cluster : clusters) {
			Vector clusterCenter = cluster.getCenter();
		...
		writer.append(new IntWritable(nearestCluster.getId()),
				new WeightedVectorWritable(1, point));
	}

	/**
	 * This is the reference k-means implementation. Given its inputs it
	...
	public static List<List<Cluster>> clusterPoints(Iterable<Vector> points,
			List<Cluster> clusters, DistanceMeasure measure, int maxIter,
			double distanceThreshold) {
		...
		while (!converged && iteration < maxIter) {
			...
			converged = runKMeansIteration(points, next, measure,
					distanceThreshold);
			iteration++;
		}
		return clustersList;
	...
	protected static boolean runKMeansIteration(Iterable<Vector> points,
			Iterable<Cluster> clusters, DistanceMeasure measure,
			double distanceThreshold) {
